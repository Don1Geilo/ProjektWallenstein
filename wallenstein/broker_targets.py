...
def _parse_price_target_item(item: Dict[str, Any]) -> Dict[str, Optional[float]]:
    ...
    FMP's price-target endpoints are not entirely consistent in their field
    naming.  Depending on the exact endpoint and parameters the response may
    use e.g. ``targetMean`` or ``mean`` for the average target price.  To make
    the parsing more robust we check multiple possible keys for each value.
    """

    mean = item.get("targetConsensus") or item.get("targetMean") or item.get("mean") or item.get("targetAverage") or item.get("priceTargetAverage")
    high = item.get("targetHigh") or item.get("high") or item.get("priceTargetHigh")
    low = item.get("targetLow") or item.get("low") or item.get("priceTargetLow")

    return {
        "target_mean": _pos(_sf(mean)),
        "target_high": _pos(_sf(high)),
        "target_low": _pos(_sf(low)),
    }
